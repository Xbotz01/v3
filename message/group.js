		...
		try {
            ...
            dha.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot Ketik ${prefix}menu', 'conversation')
             ...
             if (!welkom.includes(anu.jid)) return
                ...
                buttonsMessage = { contentText: `${teks}`, footerText: 'BIASAKAN BACA DESKIPSI GRUP TERLEBIH DAHULU', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
            ...
            if (!welkom.includes(anu.jid)) return
                ...
                buttons = [{buttonId: `#y`,buttonText:{displayText: 'SI BEBAN OUT'},type:1}]
                imageMsg = (await dha.prepareMessageMedia((buff), 'imageMessage', {thumbnail: buff})).imageMessage
                buttonsMessage = { contentText: `${out}`, footerText: 'akhirnya beban out juga 🗿', imageMessage: imageMsg, buttons: buttons, headerType: 4 }
                prep = await dha.prepareMessageFromContent(mdata.id,{buttonsMessage},{})
                dha.relayWAMessage(prep)
            }
		} catch (e) {
